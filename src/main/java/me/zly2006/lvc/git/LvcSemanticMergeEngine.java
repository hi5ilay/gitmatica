...
final class LvcSemanticMergeEngine
{
    ...
    private static LvcMergeBlockPayload choosePayload(String chunkKey, int maskIndex, LvcMergeBlockPayload basePayload,
                                                      ...
                                                      @Nullable LvcProjectService.BranchMergeConflictResolution conflictResolution) throws LvcMergeConflictException
    {
        String baseState = canonicalState(basePayload);
        String currentState = canonicalState(currentPayload);
        String sourceState = canonicalState(sourcePayload);

        if (Objects.equals(currentState, sourceState))
        {
            if (payloadEquals(currentPayload, basePayload) && !payloadEquals(sourcePayload, basePayload))
            {
        ...
        if (Objects.equals(currentState, baseState))
        {
            return sourcePayload;
        }

        if (Objects.equals(sourceState, baseState))
        {
            return currentPayload;
        }
    ...
    private static boolean payloadEquals(LvcMergeBlockPayload left, LvcMergeBlockPayload right)
    {
        return Objects.equals(left.blockState(), right.blockState()) &&
                Arrays.equals(left.blockEntityNbt(), right.blockEntityNbt()) &&
                Objects.equals(left.pendingBlockTicks(), right.pendingBlockTicks()) &&
                Objects.equals(left.pendingFluidTicks(), right.pendingFluidTicks());
    }
