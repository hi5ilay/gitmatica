...
- Tracked-content hashes match Litematica's block-state-only schematic verifier model for MVP: they include tracked block IDs/states and ignore all block entity NBT, entity payload, and scheduled tick payload. Full object bytes still store full hidden payload for restore/export.
...
3. Stage `lvc.json`, `indexes/*.lvcidx`, new object files, and `.gitignore`.
