...
LVC stores full canonical snapshots for restore/export, including normalized block entity NBT when available. MVP tracked content should match the user-visible Litematica schematic verifier model: block IDs and block states count as tracked changes, while block entity NBT, entity payload, and scheduled tick payload are stored but hidden from dirty state, diffs, no-op commit detection, and merge conflicts. It should not attempt to preserve random tick future state, entity scheduler internals, block entity scheduler internals, neighbor update queues, mod task queues, server event queues, or other transient simulation internals.
