...
- Tracked hashes ignore block entity NBT like Litematica's block-state-only schematic verifier, while full hashes still store the full payload.
...
- Semantic selected-commit checkout runs a server-authoritative preflight scan, checks out the commit, restores tracked blocks/block entities into the integrated-server world, and refreshes the overlay.
- Semantic branch checkout from the project page branch dropdown is implemented. Same-tip branch switches attach to the selected branch without restore and preserve uncommitted changes; different-tip switches use the same frozen server-authoritative checkout preflight/restore path as selected-commit checkout and confirm before overwriting dirty Git/world state.
- Semantic Clear Area runs a server-authoritative preflight scan, clears immediately when clean, requires confirmation when dirty, clears only tracked blocks to air, and refreshes verifier against the existing overlay when valid.
- Semantic Discard Changes requires confirmation, freezes scheduled activity in the tracked area, scans target chunks chunk-by-chunk, restores mismatched block states inline, ignores block-entity-only drift for dirty/no-op parity with tracked hashes, reasserts restored blocks and their tracked neighbors to stabilize pane/fence/wall-style shape drift, schedules a delayed authoritative server block update for every tracked position to clear client/verifier desync, resets Git files when needed, and refreshes verifier against the existing overlay when valid.
...
- Semantic selected-commit checkout and branch checkout now use a frozen server-authoritative preflight plus the discard-style scan/restore/stabilize/client-sync path. Semantic pull restore still needs the same hardening. Scheduled tick capture/restore, entity semantics, and Litematica paste-parity testing remain unresolved before treating restore paths as production-correct.
...
- Later MVP world-I/O optimization: keep semantic storage in project-relative `LvcChunkCoordinate` chunks, but plan capture/restore/scan execution by translated real-world `ChunkPos` columns so unloaded chunk handling, neighbor chunk requirements, and world access locality match Minecraft/Litematica behavior more closely.
...
- Later MVP freeze optimization: replace discard freeze's exact packed block-position hash set with real chunk-section -> bitset masks. Current freeze startup is `O(tracked blocks)` to build the exact multi-subregion union; unfreeze is cheap because it mostly removes JVM-side active map entries and replays/drops suppressed ticks/events.
- Scheduled tick capture. Discard Changes now suppresses queued/new scheduled block/fluid ticks, nearby random ticks, and block events inside tracked masks while it runs, and drops that suppressed work only if discard actually mutates/resets. That is not the same as capturing/restoring committed scheduled tick state.
- Entity capture/restore. Next planned model: copy Litematica's non-player entity storage using project-relative entity NBT, keep all entity bytes out of tracked hashes/diffs/no-op detection, add stored entities to export/overlay schematics, and on restore clear live non-player entities inside tracked masks before materializing stored entity NBT.
...
- Pull restore can still trigger redstone contraptions mid-restore because it has not moved to the discard/selected-checkout/branch-checkout freeze/scan-restore path yet.
...
- Avoid firing neighbor updates/redstone ticks mid-restore when the intended operation is restoring a static committed schematic state. Use the discard/selected-checkout/branch-checkout freeze/scan-restore path as the current reference for future pull restore work.
...
- `src/main/java/me/zly2006/lvc/task/LvcSemanticCheckoutTask.java`
- `src/main/java/me/zly2006/lvc/task/LvcSemanticDiscardTask.java`
- `src/main/java/me/zly2006/lvc/world/LvcWorldFreezeService.java`
- `src/main/java/me/zly2006/lvc/task/LvcSemanticClearTask.java`
...
- Semantic selected-commit checkout restores tracked block and block entity content in singleplayer/integrated-server worlds.
- Semantic branch checkout restore from the project page branch dropdown restores tracked block and block entity content in singleplayer/integrated-server worlds.
...
- Define whether LVC tracks entities by default.
...
- Add a world-operation planning layer that groups translated tracked positions by real Minecraft `ChunkPos` before reading or mutating the world.
